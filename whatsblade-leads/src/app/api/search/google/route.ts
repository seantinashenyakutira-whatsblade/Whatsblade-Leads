  ...
  try {
    ...
    const detailed = await Promise.all(
      results.slice(0, 30).map(async (place) => {
        ...
        const email = ((p as unknown as Record<string, unknown>).emailAddresses as Array<{ emailAddress: string }> | undefined)?.[0]?.emailAddress;
        ...
        return {
          ...
          city: p.formattedAddress?.split(',').slice(-2, -1)[0]?.trim() || null,
          country: p.formattedAddress?.split(',').pop()?.trim() || null,
